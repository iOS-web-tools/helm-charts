...
helm repo add webtor http://127.0.0.1:8080
#helm repo add ios-web-tools https://ghcr.io/ios-web-tools
helm repo add pascaliske https://charts.pascaliske.dev
mkdir ~/.kube
sudo microk8s kubectl config view --raw > ~/.kube/config
chmod 0600 ~/.kube/config
curl -L 'https://github.com/roboll/helmfile/releases/download/v0.143.0/helmfile_linux_arm64' > helmfile
chmod +x helmfile
sudo mv ./helmfile /usr/local/bin/
curl -L  'http://127.0.0.1:8080/helmfile.yaml' > helmfile.yaml
helmfile apply
